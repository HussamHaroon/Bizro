...
def render_report_html(report: dict) -> str:
    ...
    # Defect 3: never print raw ISO dates — prefer the payload's formatted
    # labels; format here when an older payload lacks them.
    period_label = period.get("label") or (
        format_range_label(period.get("start"), period.get("end"))
        if period.get("start") and period.get("end")
        else ""
    )
    generated_label = report.get("generated_at_display") or format_datetime_label(
        report.get("generated_at")
    )

    # Defect 4: honest runtime attribution — the model id ACTUALLY used, and
    # the provider taken from the payload (derived from the DASHSCOPE_BASE_URL
    # host at generation time) or re-derived from the current env for older
    # payloads. A brand is claimed only when the host actually says so; a mock
    # report ran no model at all and says so.
    model = report.get("model")
    provider = report.get("model_provider") or provider_from_base_url(
        os.environ.get("DASHSCOPE_BASE_URL", DEFAULT_BASE_URL)
    )
    if mock:
        attribution = "Model: none — mock report, generated without a live model call"
    elif model:
        attribution = f"Generated by {_esc(model)} via {_esc(provider)}"
    else:
        attribution = "Generated by Bizro's deterministic report pipeline"
    ...
    def _flag_li(f: dict) -> str:
        # Defect 2: every printed flag carries refs to the entries it flags —
        # or an honest short reason; never an untraceable "0 refs".
        refs = [str(r) for r in (f.get("refs") or [])]
        if refs:
            shown = ", ".join(refs[:3])
            more = f" (+{len(refs) - 3} more)" if len(refs) > 3 else ""
            trace = f"refs: {_esc(shown + more)}"
        else:
            trace = _esc(
                f.get("refs_reason") or "no linked entry ids available for this flag"
            )
        return (
            f"<li>{_esc(f.get('note_en'))} — {trace} — "
            f"<span class='ur'>{_esc(f.get('note_ur'))}</span></li>"
        )

    flags_html = "".join(_flag_li(f) for f in report.get("red_flags", [])) or "<li>None</li>"
...
<div class="card">
  ...
  <div class="sub">{_esc(merchant)} · {_esc(period_label)}</div>
...
<div class="card"><div class="cardhead"><strong>Line items (audit trail)</strong>{_stamp()}</div>
  ...
  {attribution} · Generated: {_esc(generated_label)}</div>
</div>
</div></body></html>"""
