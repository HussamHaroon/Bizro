/* ScreenHeader — one per screen (design.md §4.7: purpose graspable in ≤5 words).
   ...
   border, ink-green slab title, plain purpose line, actions right.
   English-only (owner directive 2026-09-04): titleUr/purposeUr are accepted
   from older call sites but never rendered. */
...
export function ScreenHeader({ title, purpose, icon, actions }: ScreenHeaderProps) {
  return (
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-3 px-5 py-4 sm:px-6">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="flex flex-wrap items-baseline gap-x-3 gap-y-0">
            <span className="font-numerals text-2xl font-bold tracking-wide text-ink-green">
              {title}
            </span>
          </h1>
          <p className="mt-0.5 text-sm text-ink-line opacity-80">{purpose}</p>
        </div>
